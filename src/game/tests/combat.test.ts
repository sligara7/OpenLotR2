/* Combat — auto-resolved field battles, army attacks, and undefended capture. */
...
test('combat: a much larger army annihilates a small one and survives', () => {
  const r = resolveBattle({ soldiers: 100 }, { soldiers: 15 }, rng());
  ...
  assertGreater(r.attackerSurvivors, 60, 'attacker keeps most of its strength');
});

test('combat: an even fight bloodies both sides heavily', () => {
  const r = resolveBattle({ soldiers: 100 }, { soldiers: 100 }, rng());
  assertGreater(r.attackerCasualties, 40, 'attacker takes heavy losses');
  assertGreater(r.defenderCasualties, 40, 'defender takes heavy losses');
});

test('combat: a defensive modifier can swing an even fight', () => {
  // Equal numbers, but the defender fights behind a x3 wall multiplier.
  const r = resolveBattle({ soldiers: 100 }, { soldiers: 100, modifier: 3 }, rng());
  assertEqual(r.winner, 'defender', 'the fortified side wins');
  assertGreater(r.attackerCasualties, r.defenderCasualties, 'attacker bleeds more');
});
...
function twoArmyWorld(aSoldiers: number, bSoldiers: number) {
  ...
  const a: Army = { id: 'a', ownerId: 'p1', col: 2, row: 2, countyId: 'home', soldiers: aSoldiers };
  const b: Army = { id: 'b', ownerId: 'p2', col: 2, row: 2, countyId: 'home', soldiers: bSoldiers };
...
test('combat: occupying an undefended hostile county captures it', () => {
  ...
  const army: Army = { id: 'a', ownerId: 'p1', col: 0, row: 0, countyId: 'n', soldiers: 40 };
...
test('combat: a garrisoned castle is NOT captured by mere occupation', () => {
  ...
  const army: Army = { id: 'a', ownerId: 'p1', col: 0, row: 0, countyId: 'h', soldiers: 40 };
